...
export default function EditProduct() {
  ...
  useEffect(() => {
    ...
    fetchProductById(Number(id))
      .then((res) => {
        const { title, price, stock, description, category, image } = res.data
        reset({ title, price, stock, description, category, image })
        // If existing image is base64, switch to upload tab to show it
        if (image?.startsWith('data:')) {
          setUploadedImage(image)
          setActiveTab('upload')
        }
        console.log(`[EditProduct] Pre-filled form for ID: ${id}`)
      })
      .catch(() => setServerError('Failed to load product data.'))
  ...
  const onSubmit = async (data: NewProductPayload) => {
    ...
    setServerError('')
    try {
      await updateProduct(Number(id), { ...data, price: Number(data.price), stock: Number(data.stock) })
      console.log(`[EditProduct] Product ${id} updated successfully`)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-md p-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-1">Edit Product</h2>
          <p className="text-gray-500 text-sm mb-6">Update the fields and save changes.</p>
